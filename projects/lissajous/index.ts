...
function draw(): void {
  ...
  for (let i = 1; i < width / box_size - 1; i++) {
    for (let j = 1; j < height / box_size - 1; j++) {
      ...
      strokeWeight(2);
      line(
        i * box_size + box_size / 2 + cos(t * i) * circle_radius / 2, j * box_size + box_size / 2 + sin(t * j) * circle_radius / 2,
        i * box_size + box_size / 2 + cos((t + time_step) * i) * circle_radius / 2, j * box_size + box_size / 2 + sin((t + time_step) * j) * circle_radius / 2
      )
    }
  }
  t += time_step
