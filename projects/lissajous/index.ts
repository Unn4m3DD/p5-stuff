...
function draw(): void {
  ...
  for (let i = 1; i < width / box_size - 1; i++) {
    ...
    strokeWeight(10);
    point(
      i * box_size + box_size / 2 + circle_radius / 2 * cos(t * i),
      box_size / 2 + circle_radius / 2 * sin(t * i)
    )
  }
  for (let i = 1; i < height / box_size - 1; i++) {
    ...
    strokeWeight(10);
    point(
      box_size / 2 + circle_radius / 2 * cos(t * i),
      i * box_size + box_size / 2 + circle_radius / 2 * sin(t * i)
    )
  }
